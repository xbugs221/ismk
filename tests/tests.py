...
from ismk.resources import DefaultResources, GroupResources, is_ordinary_string
from ismk.settings.enums import RerunTrigger

from ismk.settings.types import Batch
from ismk.shell import shell
from ismk.exceptions import AmbiguousRuleException, WorkflowError
...
def test_github_issue_14():
    ...
    assert os.listdir(os.path.join(tmpdir, ".ismk", "scripts"))
    shutil.rmtree(tmpdir, ignore_errors=ON_WINDOWS)

    # And not here
    tmpdir = run(dpath("test_github_issue_14"), cleanup=False, cleanup_scripts=True)
    assert not os.listdir(os.path.join(tmpdir, ".ismk", "scripts"))
    shutil.rmtree(tmpdir, ignore_errors=ON_WINDOWS)
...
#         shellcmd=(
#             "ismk -j 10 --cluster=./sbatch --cluster-cancel=./scancel.sh "
...
def test_set_threads():
    run(
        ...
        shellcmd='ismk --executor local --set-threads "a=max(input.size, 5)" -c 5 --verbose',
    )
...
def test_set_resources_complex_profile():
    run(
        ...
        shellcmd="ismk -c1 --verbose --profile test-profile",
    )
...
def test_set_resources_complex_cli():
    run(
        dpath("test_set_resources_complex"),
        shellcmd="ismk -c1 --verbose --set-resources \"a:slurm_extra='--nice=150 --gres=gpu:1'\"",
    )
...
def test_ancient_cli():
    run(
        ...
        shellcmd="ismk --consider-ancient A=0 B=x --cores 1",
    )
...
def test_no_temp():
    run(dpath("test_no_temp"), shellcmd="ismk -c1 --no-temp")
...
def test_config_merging():
    run(
        ...
        shellcmd='ismk -j 1 --configfile config_cmdline_01.yaml config_cmdline_02.yaml --config "block={bowser: cmdline_bowser}" "block={toad: cmdline_toad}"',
    )
...
def test_config_replacing():
    run(
        ...
        shellcmd='ismk -j 1 --configfile cli-config.yaml --config "value=value2" --replace-workflow-config',
    )
...
def test_config_replacing_nocli():
    run(
        ...
        shellcmd="ismk -j 1 --replace-workflow-config",
    )
...
def test_script_python():
    ...
    run(
        ...
        cleanup=False,
        tmpdir=tmpdir,
        shellcmd="ismk -c1 --rerun-triggers mtime",
    )
...
#     The shell snippet in the SMK file will fail the first time
...
def test_group_job_resources_with_pipe(mocker):
    import copy
    from ismk.interfaces.executor_plugins.executors.real import RealExecutor

    spy = mocker.spy(GroupResources, "basic_layered")

    # Cluster jobs normally get submitted with cores=all, but for testing purposes,
    # the system may not actually have enough cores to run the ismk subprocess
...
def test_convert_to_cwl():
    ...
    shell(
        "cd {workdir}; PYTHONPATH={src} python -m ismk --export-cwl workflow.cwl",
        src=Path.cwd(),
    )
...
def test_default_resources():
    from ismk.resources import DefaultResources
...
def test_filegraph():
    ...
    shell("cd {workdir}; python -m ismk --filegraph > {dot_path}")
...
def test_github_issue52():
    ...
    run(dpath("test_github_issue52"), snakefile="other.ismk")
...
@patch("ismk.io._IOFile.retrieve_from_storage", AsyncMock(side_effect=Exception))
...
def test_jupyter_notebook_draft():
    from ismk.settings.types import NotebookEditMode
...
def test_generate_unit_tests():
    run(
        ...
        shellcmd="ismk --generate-unit-tests ../../.tests/unit --directory .tests/integration",
        no_tmpdir=True,
        cleanup=False,
    )
    sp.check_call(
...
def test_module_complex():
    ...
    with patch("ismk.utils.min_version", return_value=True):
...
def test_github_issue1069():
    run(
        ...
        shellcmd="ismk -c1 --resources mem_mb=16423",
    )
...
def test_max_jobs_per_timespan():
    run(dpath("test01"), shellcmd="ismk --max-jobs-per-timespan 2/1s --cores 3")
...
def test_github_issue1384():
    try:
        ...
        shell(
            """
            cd {tmpdir}
            python -m ismk --generate-unit-tests
            pytest -v .tests/unit
            """
        )
    finally:
...
def test_workflow_profile():
    ...
    run(
        ...
        shellcmd=f"ismk --profile {general_profile} -c1",
    )
...
def test_no_workflow_profile():
    ...
    run(
        test_path,
        snakefile="workflow/Snakefile",
        shellcmd=f"ismk --profile {general_profile} --workflow-profile none -c1",
    )
...
def test_issue2574():
    ...
    p = sp.Popen(
        f"ismk -s {snakefile} --configfile {configfile} --lint",
        shell=True,
        stdout=sp.PIPE,
        stderr=sp.PIPE,
    )
...
def test_default_storage_provider_none():
    run(dpath("test01"), shellcmd="ismk --default-storage-provider none -c3")
...
def test_set_resources_complex():
    run(
        dpath("test05"),
        shellcmd="ismk --verbose -c1 --set-resources \"compute1:slurm_extra='--nice=10'\"",
    )
...
def test_set_resources_human_readable():
    run(
        dpath("test05"),
        shellcmd="ismk -c1 --set-resources \"compute1:runtime='50h'\"",
    )
...
def test_list_input_changes():
    run(dpath("test01"), shellcmd="ismk --list-input-changes", check_results=False)
...
@skip_on_windows  # OS agnostic
def test_summary():
    run(dpath("test01"), shellcmd="ismk --summary", check_results=False)
...
def test_failed_intermediate():
    # see https://github.com/ismk/ismk/pull/2966#issuecomment-2558133016
...
def test_issue3361_pass():
    run(
        ...
        shellcmd="ismk --sdm apptainer --cores 1",
        targets=["all"],
    )
...
def test_resource_quoting_profile():
    run(
        ...
        shellcmd="ismk --executor cluster-generic "
...
def test_resource_quoting_cli():
    run(
        dpath("test_resource_quoting"),
        shellcmd="ismk --set-resources \"a:test='foo'\" "
...
def test_default_resource_quoting_cli():
    run(
        dpath("test_resource_quoting"),
        shellcmd="ismk --default-resources \"test='foo'\" "
        "--executor cluster-generic "
        "--cluster-generic-submit-cmd ./qsub --jobs 1",
    )
...
def test_keep_local():
    with tempfile.TemporaryDirectory() as tmpdir:
        snakefile = os.path.join(dpath("test_local_and_retrieve"), "keep_local.ismk")
        local_img = os.path.join(
            tmpdir,
            ".ismk/storage/http/github.com/ismk/ismk/blob/main/images/logo.png",
        )

        p = sp.check_output(
            ["ismk", "-s", snakefile, "-c1", "keep_local_default.flag"], cwd=tmpdir
        )
        assert not os.path.exists(local_img)

        p = sp.check_output(
            ["ismk", "-s", snakefile, "-c1", "keep_local_false.flag"], cwd=tmpdir
        )
        assert not os.path.exists(local_img)
        ...
        shutil.rmtree(os.path.join(tmpdir, ".ismk", "storage", "http"))

        p = sp.check_output(
            ["ismk", "-s", snakefile, "-c1", "keep_local_true.flag"], cwd=tmpdir
        )
        assert os.path.exists(local_img)
        shutil.rmtree(os.path.join(tmpdir, ".ismk", "storage", "http"))

        p = sp.check_output(
            ["ismk", "-s", snakefile, "-c1", "keep_local_true_directive.flag"],
            cwd=tmpdir,
        )
...
def test_retrieve():
    with tempfile.TemporaryDirectory() as tmpdir:
        snakefile = os.path.join(dpath("test_local_and_retrieve"), "retrieve.ismk")
        local_img = os.path.join(
            tmpdir,
            ".ismk/storage/http/github.com/ismk/ismk/blob/main/images/logo.png",
        )
        ...
        p = sp.check_output(
            ["ismk", "-s", snakefile, "-c1", "retrieve_false.flag"], cwd=tmpdir
        )
        assert not os.path.exists(local_img)

        p = sp.check_output(
            ["ismk", "-s", snakefile, "-c1", "retrieve_false_directive.flag"],
            cwd=tmpdir,
        )
        assert not os.path.exists(local_img.replace("http", "http_ret"))

        p = sp.check_output(
            ["ismk", "-s", snakefile, "-c1", "retrieve_default.flag", "--force"],
            cwd=tmpdir,
        )
        assert os.path.exists(local_img)
        ...
        p = sp.check_output(
            ["ismk", "-s", snakefile, "-c1", "retrieve_true.flag"], cwd=tmpdir
        )
        assert os.path.exists(local_img)
...
def test_github_issue3556():
    run(dpath("test_github_issue3556"), shellcmd="ismk --dag mermaid-js >dag.mmd")
...
def test_checkpoint_until():
    run(dpath("test_checkpoint_until"), shellcmd="ismk --until B1 --cores 1")


def test_checkpoint_omit_from():
    run(
        dpath("test_checkpoint_omit_from"),
        shellcmd="ismk --omit-from B1 --cores 1",
    )

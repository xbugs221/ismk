...
import pytest
from ismk.deployment.conda import get_env_setup_done_flag_file
...
# 'C:\\Users\\RUNNER~1\\AppData\\Local\\Temp\\ismk-2q4osog0\\test-env.yaml'
@skip_on_windows
@conda
def test_conda_create_envs_only():
    ...
    env_dir = next(
        (p for p in Path(tmpdir, ".ismk", "conda").iterdir() if p.is_dir()), None
    )
...
def test_deploy_hashing():
    ...
    assert len(next(os.walk(os.path.join(tmpdir, ".ismk/conda")))[1]) == 2
...
def test_wrapper_local_git_prefix():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        repo = git.Repo.clone_from(
            "https://github.com/ismk/ismk-wrappers", tmpdir
        )
...
def test_resource_string_in_cli_or_profile():
    ...
    run(
        test_path,
        snakefile="Snakefile",
        shellcmd=f"ismk --workflow-profile {profile} -c1 --default-resources slurm_account=foo other_resource='--test'",
    )
...
def test_containerize_checkpoint():
    ...
    try:
        tmpdir = run(
            ...
            shellcmd="ismk --containerize > Dockerfile",
            check_results=False,
            cleanup=False,
            deployment_method={DeploymentMethod.CONDA},
        )

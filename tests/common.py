...
from ismk.interfaces.executor_plugins.settings import SharedFSUsage
from ismk.interfaces.executor_plugins.registry import ExecutorPluginRegistry

from ismk import api
from ismk.common import ON_WINDOWS
from ismk.report.html_reporter import ReportSettings
from ismk.resources import ResourceScopes
from ismk.scheduling.milp import SchedulerSettings
from ismk.settings import types as settings
...
def run(
    ...
    config=dict(),
    targets=set(),
    container_image=os.environ.get("CONTAINER_IMAGE", "ismk/ismk:latest"),
    shellcmd=None,
    sigint_after=None,
    ...
    local_storage_prefix=Path(".ismk/storage"),
    ...
    if set_pythonpath:
        # Enforce current workdir (the ismk source dir) to also be in PYTHONPATH
    ...
    if tmpdir is None:
        ...
        tmpdir = os.path.join(
            tempfile.gettempdir(), f"ismk-{original_dirname}-{tmpdir}"
        )
        os.mkdir(tmpdir)
    ...
    # run ismk
    if shellcmd:
        if not shellcmd.startswith("ismk"):
            raise ValueError("shellcmd does not start with ismk")
    ...
    if shouldfail:
        ...
        if not success:
            if ismk_api is not None and exception is not None:
                ismk_api.print_exception(exception)
            print("Workdir:")
            print_tree(tmpdir, exclude=".ismk/conda")

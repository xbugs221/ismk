...
from ismk.script import get_source, ScriptBase, PythonScript, RScript
from ismk.logging import logger
from ismk.common import is_local_file
from ismk.common import ON_WINDOWS
from ismk.sourcecache import SourceCache, infer_source_file
...
class JupyterNotebook(ScriptBase):
    ...
    def insert_preamble_cell(self, preamble, notebook):
        ...
        preamble_cell["metadata"]["tags"] = ["ismk-job-properties"]
    ...
    def remove_preamble_cell(self, notebook):
        preambles = [
            ...
            if "ismk-job-properties" in cell["metadata"].get("tags", [])
        ]
        if len(preambles) > 1:
            raise WorkflowError(
                "More than one ismk preamble cell found in notebook. "

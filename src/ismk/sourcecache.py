...
from ismk.exceptions import WorkflowError, SourceFileError
from ismk.common.git import split_git_path
...
class StringSourceFile(SourceFile):
    """SourceFile implementation for in-memory Snakefile content.

    This allows passing Snakefile content as a string instead of requiring
    a file on disk, enabling programmatic workflow definition.
    """

    def __init__(self, content: str, name: str = "<string>"):
        """Initialize a StringSourceFile.

        Args:
            content: The Snakefile content as a string
            name: Optional name for the source (used in error messages and compilation)
        """
    ...
    def exists(self):
        """String sources always 'exist' since content is in memory."""
        return True

    def absolute(self):
        """Return self as string sources have no filesystem path."""
        return self

    @property
    def is_local(self):
        # Treat as local since content is already in memory
        return True
...
class SourceCache:
    cache_whitelist = [
        r"https://raw.githubusercontent.com/ismk/ismk-wrappers/\d+\.\d+.\d+"
    ...
    def _open(self, source_file: SourceFile, mode, encoding=None):
        ...
        if isinstance(source_file, StringSourceFile):
            # For StringSourceFile, return a StringIO or BytesIO object
            if "b" in mode:
                return io.BytesIO(source_file.content.encode(encoding or "utf-8"))
            else:
                return io.StringIO(source_file.content)

...
from ismk.interfaces.common.utils import lchmod
from ismk.interfaces.common.utils import lutime as lutime_raw
from ismk.interfaces.common.utils import not_iterable
from ismk.interfaces.common.io import AnnotatedStringInterface
from ismk.interfaces.storage_plugins.io import (
    WILDCARD_REGEX,
...
class IOCache(IOCacheStorageInterface):
    ...
    async def mtime_inventory(
        self, jobs: "collections.abc.Iterable[ismk.jobs.Job]", n_workers=8
    ):
...
def IOFile(file, rule: Union["ismk.rules.Rule", None] = None):
...
class _IOFile(str, AnnotatedStringInterface):
    ...
    if TYPE_CHECKING:
        ...
        def __init__(self, file, rule: Optional["ismk.rules.Rule"]):
            self._is_callable: bool
            self._file: str | AnnotatedString | Callable[[Namedlist], str]
            self.rule: ismk.rules.Rule | None
    ...
    def check(self):
        ...
        if self._file == "":
            if self.rule is not None:
                ...
                spec = ""
            logger.warning(
                f"{spec}Empty file path encountered. SMK cannot understand this. "
    ...
    async def mtime_uncached(self, skip_storage: bool = False):
        ...
        try:
            ...
            def get_dir_mtime():
                ...
                return os.stat(
                    os.path.join(self.file, ".ismk_timestamp"),
            ...
            raise WorkflowError(
                ...
                "It could be that a concurrent process has deleted it while SMK "
    ...
    async def touch_storage_and_local(self):
        from ismk.interfaces.storage_plugins.storage_object import (
            StorageObjectTouch,
        )
    ...
    def touch(self, times=None):
        ...
        try:
            if self.is_directory:
                file = os.path.join(self.file, ".ismk_timestamp")
...
def temp(value, group_jobs=False):
    ...
    A flag for an intermediate file that only lives on the compute node executing the group jobs and not accessible from the main ismk job.
    e.g. for what some HPC call "local scratch". This will cause ismk to automatically group rules on the same compute note.
    """
...
def sourcecache_entry(value, orig_path_or_uri):
    from ismk.sourcecache import SourceFile
...
def report(
    ...
    patterns -- Wildcard patterns for selecting files if a directory is given (this is used as
               input for ismk.io.glob_wildcards). Pattern shall not include the path to the
               directory itself.
    """
    return flag(
        value,
...
def expand(*args, **wildcard_values):
    ...
    **wildcard_values -- the wildcards as keyword arguments
    ...
    """
    from ismk.path_modifier import PATH_MODIFIER_FLAG
...
class AttributeGuard:
    ...
    def __call__(self, *args, **kwargs):
        ...
        raise AttributeError(
            f"{self.name}() cannot be used on ismk input, output, resources etc.; "
    ...
    def __init__(
        ...
        if toclone is not None:
            if custom_map is not None:
                ...
                self.extend(
                    # use original query if storage is not retrieved by ismk
                    (
                        (
                            str(x)

...
import time
import json
from typing import Iterable, List, Mapping, Optional, Set, Union, Dict, TYPE_CHECKING
import uuid
...
from ismk.common.typing import AnySet
from ismk.io.flags.access_patterns import AccessPattern
from ismk.io.fmt import fmt_iofile
from ismk.rules import Rule
from ismk.settings.types import DeploymentMethod

from ismk.interfaces.executor_plugins.dag import DAGExecutorInterface
from ismk.interfaces.report_plugins.interfaces import DAGReportInterface
from ismk.interfaces.storage_plugins.storage_object import StorageObjectTouch
...
if TYPE_CHECKING:
    from ismk import workflow as _workflow
...
from ismk.settings.enums import RerunTrigger, StrictDagEvaluation
...
from ismk.settings.enums import PrintDag
from ismk.io import (
    _IOFile,
    PeriodicityDetector,
    flags,
...
from ismk.settings.types import SharedFSUsage
from ismk.logging import logger
from ismk.output_index import OutputIndex
from ismk.sourcecache import LocalSourceFile, SourceFile
...
class DAG(DAGExecutorInterface, DAGReportInterface, DAGSchedulerInterface):
    ...
    def incomplete_external_jobid(self, job) -> Optional[str]:
        ...
        if len(jobids) == 1:
            ...
            raise WorkflowError(
                ...
                "cannot be resumed. Execute SMK with --rerun-incomplete "
    ...
    def update_priority(self):
        ...
        def prioritized(job):
            return job.rule in self.priorityrules or not self.priorityfiles.isdisjoint(
                job.output
            )
        ...
        for group in self._group.values():
            for job in group:
                ...
                if outside_jobs_all:
                    ...
                    def fmt_outside(jobs):
                        return ",".join(sorted(set(j.rule.name for j in jobs)))
    ...
    def handle_pipes_and_services(self):
        ...
        for job in self.needrun_jobs():
            ...
            if has_nodelocal and not self.workflow.local_exec:
                ...
                # NOTE do NOT create usergroup in the node's local inst of ismk lest the resources get merge, see: resources.py: class GroupResources
    ...
    def get_sources(self):
        ...
        try:
            ...
            if "fatal: not a git repository" in e.stderr.decode().lower():
                ...
                raise WorkflowError(
                    "Error executing git (SMK requires git to be installed for "

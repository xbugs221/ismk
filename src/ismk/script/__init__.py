...
from ismk import io as io_
from ismk import sourcecache
from ismk.common import (
    MIN_PY_VERSION,
    ON_WINDOWS,
    get_report_id,
    get_ismk_searchpaths,
)
from ismk.deployment import singularity
from ismk.exceptions import WorkflowError
...
class BashEncoder:
    ...
    def __init__(
        ...
        """namedlists is a list of strings indicating the ismk object's member
        variables which are encoded as Namedlist.
        dicts is a list of strings indicating the ismk object's member variables
        that are encoded as dictionaries.
        Prefix is the prefix for the bash variable name(s) e.g., ismk_input
        """
        if dicts is None:
            dicts = []
    ...
    def encode_ismk(self, ismk: SMK) -> str:
        """Turn a ismk object into a collection of bash associative arrays"""
        arrays = []
        main_aa = dict()
        for var in vars(ismk):
            val = getattr(ismk, var)
    ...
    def encode_namedlist(cls, named_list: io_.Namedlist) -> str:
        ...
        # Add the same items keyed by index and also by name
        for i, (name, val) in enumerate(named_list._allitems()):
            items.append((str(i), val))
            if name is not None:
                items.append((name, val))

        return cls.dict_to_aa(items)
...
class PythonScript(ScriptBase):
    ...
    def generate_preamble(
        ...
        ismk = pickle.dumps(ismk)
        # Obtain search path for current ismk module.
        # The module is needed for unpickling in the script.
        # We append it at the end (as a fallback).
        searchpaths = get_ismk_searchpaths()
        if container_img is not None:
            searchpaths = singularity.get_ismk_searchpath_mountpoints()
        ...
        shell_exec_stmt = (
            ...
            else f"from ismk.shell import shell; shell.executable({shell_exec});"
        )
        ...
        preamble = f"""
            ...
            import pickle;
            from ismk import script;
            script.ismk = pickle.loads({ismk});
            del script;
            from ismk.logging import logger;
        ...
        return "\n".join(
            [
                "######## ismk preamble start (automatically inserted, do not edit) ########",
    ...
    def generate_preamble(
        ...
        ismk <- SMK(
            ...
            source = function(...){{
                ...
                is_url <- grepl("^https?://", ismk@scriptdir)
                file <- ifelse(is_url, file.path(ismk@scriptdir, ...), ...)
                if (!is_url) setwd(ismk@scriptdir)
                source(file)
            }}
        )
        {preamble_addendum}
...
class RMarkdown(ScriptBase):
    ...
    def write_script(self, preamble, fd):
        # Insert SMK object after the RMarkdown header
        code = self.source
...
class RustScript(ScriptBase):
    ...
    def generate_preamble(
        ...
        # ismk's namedlists will be encoded as a dict
        ...
        json_string = json.dumps(dict(ismk))

        return textwrap.dedent(
            """
            json_typegen::json_typegen!("SMK", r###"{json_string}"###, {{
                "/bench_iteration": {{
            ...
            lazy_static::lazy_static! {{
                ...
                static ref ismk: SMK = {{
                    let s: SMK = serde_json::from_str(r###"{json_string}"###).expect("Failed parsing ismk JSON");
                    s
                }};
            }}
...
class BashScript(ScriptBase):
    ...
    def generate_preamble(
        ...
        preamble = encoder.encode_ismk(ismk)
        return preamble

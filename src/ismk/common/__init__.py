...
from ismk import __version__
from ismk.interfaces.common.exceptions import WorkflowError


MIN_PY_VERSION: Tuple[int, int] = (3, 7)
UUID_NAMESPACE = uuid.uuid5(uuid.NAMESPACE_URL, "https://ismk.readthedocs.io")
...
# see https://github.com/ismk/ismk/issues/2097
IO_PROP_LIMIT = 100
...
def async_run(coroutine):
    ...
    coroutine.
    .. seealso::
         https://github.com/ismk/ismk/issues/1105
    ...
    try:
        ...
        coroutine.close()
        raise WorkflowError(
            "Error running coroutine in event loop. SMK currently does not "
            "support being executed from an already running event loop. "
            "If you run SMK e.g. from a Jupyter notebook, make sure to spawn a "
            "separate process for SMK.",
            e,
        )
...
RULEFUNC_CONTEXT_MARKER = "__is_ismk_rule_func"
...
def parse_uri(path_or_uri):
    ...
    try:
        ...
        # SMK sees a lot of URIs which are not supported by smart_open yet
...
def get_container_image():
    return f"ismk/ismk:v{get_last_stable_version()}"

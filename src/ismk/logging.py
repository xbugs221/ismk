...
if TYPE_CHECKING:
    ...
    from ismk.settings.types import OutputSettings
...
def get_event(record: logging.LogRecord) -> Optional[LogEvent]:
    """Get ismk log event from a log record."""
...
class DefaultFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        """
        Override format method to format SMK-specific log messages.
        """
...
class ColorizingTextHandler(logging.StreamHandler):
    """
    Custom handler that combines colorization and SMK-specific formatting.
    """
...
class LoggerManager:
    ...
    def setup_logfile(self, workdir: Optional[os.PathLike] = None) -> None:
        if self.settings.enable_file_logging and not self.settings.dryrun:
            if workdir:
                logdir = os.path.join(workdir, ".ismk", "log")
            else:
                logdir = os.path.join(".ismk", "log")
            try:

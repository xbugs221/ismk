...
class StorageSettings(SettingsBase, StorageSettingsExecutorInterface):
    ...
    local_storage_prefix: Path = Path(".ismk/storage")
...
class SchedulingSettings(SettingsBase):
    ...
    greediness:
        ...
        Deprecated, use ismk.scheduling.greedy.Settings instead and pass it as greedy_scheduler_settings to DAGApi.execute_workflow().
    subsample:
...
class SecuritySettings(SettingsBase):
    """Security settings for workflow protection."""

    metadata_encryption_key: Optional[bytes] = None
    """
    Encryption key for metadata protection.

    When provided, SMK will encrypt sensitive fields (code, shellcmd, params)
    in metadata files to protect proprietary workflows and business logic.
    ...
    Example:
        security_settings = SecuritySettings(metadata_encryption_key=YOUR_KEY)
    """

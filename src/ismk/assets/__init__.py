...
# be called before the setup (and dependency deployment) of the ismk package.
...
class Asset:
    ...
    def get_content(self) -> bytes:
        ...
        req = urllib.request.Request(url, headers={"User-Agent": "ismk"})
        err = None
        for _ in range(6):
            try:
...
class Assets:
    ...
    spec: Dict[str, Asset] = {
        "ismk/LICENSE.md": Asset(
            url="https://raw.githubusercontent.com/ismk/ismk/main/LICENSE.md",
    ...
    @classmethod
    def deploy(cls) -> None:
        # this has to work from setup.py without being able to load the ismk
        # modules.
    ...
    def base_path(cls) -> Path:
        # this is called from within ismk, so we can use importlib.resources
        if cls._base_path is None:
            cls._base_path = importlib.resources.files("ismk.assets") / "data"

...
from ismk.io.flags.access_patterns import AccessPatternFactory
from ismk.common.workdir_handler import WorkdirHandler
from ismk.pathvars import Pathvars
from ismk.settings.types import (
    ConfigSettings,
    DAGSettings,
    DeploymentMethod,
    DeploymentSettings,
...
from ismk.settings.enums import Quietness
from ismk.interfaces.executor_plugins.workflow import WorkflowExecutorInterface
from ismk.interfaces.executor_plugins.cli import (
    SpawnedJobArgsFactoryExecutorInterface,
)
from ismk.interfaces.common.utils import lazy_property
from ismk.interfaces.executor_plugins.settings import ExecutorSettingsBase
from ismk.interfaces.executor_plugins.registry.plugin import (
    Plugin as ExecutorPlugin,
)
from ismk.interfaces.executor_plugins.settings import ExecMode
...
from ismk.interfaces.report_plugins.registry.plugin import Plugin as ReportPlugin
...
from ismk.interfaces.scheduler_plugins.registry.plugin import (
    Plugin as SchedulerPlugin,
)

from ismk.scheduling.greedy import SchedulerSettings as GreedySchedulerSettings
from ismk.logging import logger, format_resources, logger_manager
from ismk.rules import Rule, Ruleorder, RuleProxy
from ismk.exceptions import (
...
from ismk.dag import DAG, ChangeType
from ismk.scheduling.job_scheduler import JobScheduler
from ismk.parser import parse
import ismk.io
from ismk.io import (
    expand,
    from_queue,
...
from ismk.utils import update_config

import ismk.wrapper
from ismk.common import (
    ON_WINDOWS,
    async_run,
    get_appdirs,
    is_local_file,
...
from ismk.utils import simplify_path
from ismk.checkpoints import Checkpoints
from ismk.resources import ParsedResource, ResourceScopes
from ismk.caching.local import OutputFileCache as LocalOutputFileCache
from ismk.caching.storage import OutputFileCache as StorageOutputFileCache
from ismk.modules import ModuleInfo, WorkflowModifier, get_name_modifier_func
from ismk.ruleinfo import InOutput, RuleInfo
from ismk.sourcecache import (
    LocalSourceFile,
    SourceCache,
    SourceFile,
    StringSourceFile,
...
from ismk.deployment.conda import Conda
from ismk import caching, sourcecache
import ismk.ioutils
import ismk.ioflags
from ismk.jobs import jobs_to_rulenames
...
class Workflow(WorkflowExecutorInterface):
    ...
    def __post_init__(self):
        ...
        """
        from ismk.storage import StorageRegistry
        ...
        self._ismk_tmp_dir = tempfile.TemporaryDirectory(prefix="ismk")
        ...
        from ismk.io import report
        from ismk.script import script
        from ismk.notebook import notebook
        from ismk.wrapper import wrapper

        _globals["shell"] = shell
        _globals["script"] = script
        _globals["notebook"] = notebook
        _globals["wrapper"] = wrapper
        ...
        _globals["report"] = report
        ismk.ioutils.register_in_globals(_globals)
        ismk.ioflags.register_in_globals(_globals)
    ...
    @property
    def ismk_tmp_dir(self) -> Path:
        return Path(self._ismk_tmp_dir.name)

    @property
    def ismk_tmp_dir(self) -> Path:
        # Alias for the temporary working directory used by ismk.
        return self.ismk_tmp_dir
    ...
    def source_cache_path(self) -> Path:
        ...
        if SharedFSUsage.SOURCE_CACHE not in self.storage_settings.shared_fs_usage:
            ...
            return self.ismk_tmp_dir / "source-cache"
        else:
            return Path(os.path.join(get_appdirs().user_cache_dir, "ismk/source-cache"))
    ...
    def upload_sources(self):
        assert self.storage_settings is not None
        with tempfile.NamedTemporaryFile(suffix="ismk-sources.tar.xz") as tf:
            ...
            if prefix and not prefix.endswith("/"):
                prefix = f"{prefix}/"
            query = f"{prefix}ismk-workflow-sources.{checksum}.tar.xz"
    ...
    @property
    def touch(self):
        import ismk.executors.touch

        return self.executor_plugin is not None and issubclass(
            self.executor_plugin.executor, ismk.executors.touch.Executor
        )
    ...
    def spawned_job_args_factory(self) -> SpawnedJobArgsFactoryExecutorInterface:
        from ismk.spawn_jobs import SpawnedJobArgsFactory
    ...
    def lint(self, json=False):
        from ismk.linting.rules import RuleLinter
        from ismk.linting.snakefiles import SnakefileLinter
    ...
    def _prepare_dag(
        ...
        if self.workflow_settings.cache is not None:
            ...
            try:
                ...
                logger.warning(
                    ...
                    "location specified. Hence, SMK will not use between workflow "
                    "caching (see "
                    "https://ismk.readthedocs.io/en/stable/executing/caching.html). "
        ...
        self.iocache = ismk.io.IOCache(self.dag_settings.max_inventory_wait_time)
        ...
        # Get encryption key from security settings if provided
        encryption_key = (
            self.security_settings.metadata_encryption_key
        ...
        self._persistence = Persistence(
            ...
            metadata_encryption_key=encryption_key,
        )
    ...
    def containerize(self):
        from ismk.deployment.containerize import containerize
    ...
    def export_cwl(self, path: Path):
        ...
        from ismk.cwl import dag_to_cwl
        import json
    ...
    def create_report(
        ...
        from ismk.report import auto_report
    ...
    def log_rulegraph(self):
        def simple_rulegraph():
            from ismk.report.rulegraph_spec import get_representatives
    ...
    def register_envvars(self, *envvars):
        ...
        if self.check_envvars and undefined:
            raise WorkflowError(
                ...
                "Please make sure that they are correctly defined before running SMK:\n"
                "{}".format("\n".join(undefined))
            )
    ...
    def include_string(
        ...
        """
        Include a snakefile from a string content.

        Args:
            content: The Snakefile content as a string
            name: Optional name for the source (used in error messages, default: "<string>")
            overwrite_default_target: Whether to overwrite the default target
            print_compilation: Whether to print the compiled code instead of executing it
        """
        snakefile = StringSourceFile(content, name)
    ...
    def configfile(self, fp):
        ...
        from ismk.common.configfile import load_configfile
    ...
    def get_rule_source(self, func):
        ...
        for func_offset, line in dis.findlinestarts(func.__code__):
            ...
            # with ismk is mapped to the 'rule: ' line and is not considered
    ...
    def rule(self, name=None, lineno=None, snakefile=None, checkpoint=False):
        ...
        def decorate(ruleinfo: RuleInfo):  # type: ignore[no-redef]
            ...
            if ruleinfo.wrapper:
                rule.conda_env = ismk.wrapper.get_conda_env(
            ...
            if ruleinfo.env_modules:
                ...
                from ismk.deployment.env_modules import EnvModules

...
from ismk import caching
from ismk.interfaces.executor_plugins.settings import ExecMode
from ismk.interfaces.executor_plugins.registry import ExecutorPluginRegistry
from ismk.interfaces.executor_plugins.utils import is_quoted, maybe_base64
from ismk.interfaces.storage_plugins.registry import StoragePluginRegistry
from ismk.interfaces.report_plugins.registry import ReportPluginRegistry
from ismk.interfaces.logger_plugins.registry import LoggerPluginRegistry
from ismk.interfaces.scheduler_plugins.registry import SchedulerPluginRegistry


import ismk.common.argparse
from ismk.api import (
    SMKApi,
    resolve_snakefile,
)
...
from ismk.settings.enums import (
    ChangeType,
    Quietness,
    RerunTrigger,
    StrictDagEvaluation,
...
from ismk.target_jobs import parse_target_jobs_cli_args
from ismk.utils import available_cpu_count, update_config
from ismk.scheduling.milp import SchedulerSettings as MILPSchedulerSettings
...
def get_argument_parser(profiles=None):
    ...
    from ismk.profiles import ProfileConfigFileParser
    ...
    parser = ismk.common.argparse.ArgumentParser(
        description="SMK is a Python based language and execution "
        "environment for GNU Make-like workflows.",
        formatter_class=ismk.common.argparse.ArgumentDefaultsHelpFormatter,
    ...
    group_exec.add_argument(
        ...
        help=(
            ...
            "By default, SMK will search for {} "
    ...
    group_exec.add_argument(
        ...
        help=(
            ...
            "https://ismk.readthedocs.io/en/stable/snakefiles/rules.html#"
    ...
    group_exec.add_argument(
        ...
        help=(
            ...
            "By default, SMK assumes a default for mem_mb, disk_mb, and tmpdir (see below). "
            ...
            "If this argument is not specified at all, SMK just uses the tmpdir resource as outlined above. "
    ...
    group_exec.add_argument(
        ...
        help=(
            ...
            "fool future invocations of ismk. Fails if a file does "
            "not yet exist. Note that this will only touch files that would "
            "otherwise be recreated by SMK (e.g. because their input "
    ...
    group_exec.add_argument(
        "--executor",
        "-e",
        help="Specify a custom executor, available via an executor plugin: ismk_executor_<name>",
    ...
    group_exec.add_argument(
        ...
        help=(
            ...
            "If not supplied, the value is set to the `.ismk` directory relative "
    ...
    group_exec.add_argument(
        ...
        "SMK executes each job on the remote compute node.",
    )
    ...
    group_report.add_argument(
        "--reporter",
        metavar="PLUGIN",
        help="Specify a custom report plugin. By default, SMK's builtin html "
    ...
    group_notebooks.add_argument(
        ...
        "will automatically be reused in non-interactive mode by SMK for subsequent jobs.",
    )
    group_notebooks.add_argument(
        "--edit-notebook",
        metavar="TARGET",
    ...
    group_utils.add_argument(
        ...
        const="text",
        choices=["text", "json"],
        help="Perform linting on the given workflow. This will print ismk "
    ...
    group_exec.add_argument(
        ...
        "use on Unix systems: `ismk --dag | dot | display`. "
    ...
    group_utils.add_argument(
        ...
        "Recommended use on Unix systems: ismk `--rulegraph | dot | display`. "
        "Note print statements in your Snakefile may interfere "
        "with visualization.",
    )
    group_utils.add_argument(
        ...
        "cyclic if a rule appears in several steps of the workflow. "
        "Use this if above option leads to a DAG that is too large. "
        "Recommended use on Unix systems: ismk --filegraph | dot | display. "
        "Note print statements in your Snakefile may interfere "
        "with visualization.",
    )
    group_utils.add_argument(
    ...
    group_utils.add_argument(
        ...
        "of given files. That means that ismk removes any tracked "
    ...
    group_behavior.add_argument(
        ...
        action="store_true",
        help=(
            "Tell SMK to assume that all input and output file existence and modification "
    ...
    group_behavior.add_argument(
        "--wrapper-prefix",
        default="https://github.com/ismk/ismk-wrappers/raw/",
    ...
    group_behavior.add_argument(
        ...
        "one (e.g. `s3`). See https://ismk.github.io/ismk-plugin-catalog "
    ...
    group_behavior.add_argument(
        "--runtime-profile",
        metavar="FILE",
        help="Profile SMK and write the output to FILE. This requires yappi "
        "to be installed.",
    )
    group_behavior.add_argument(
        "--local-groupid",
    ...
    group_cluster.add_argument(
        ...
        "Defaults to https://hub.docker.com/r/ismk/ismk, tagged with "
        "the same version as the currently running SMK instance. "
        "Note that overwriting this value is up to your responsibility. "
        "Any used image has to contain a working ismk installation "
    ...
    group_cluster.add_argument(
        ...
        "`$ ismk --cluster 'sbatch --dependency {dependencies}'`. "
        ...
        "also recommended to avoid SMK trying to download output files "
    ...
    group_conda.add_argument(
        ...
        "to the `.ismk` directory relative to the invocation directory. "
    ...
    from ismk.deployment.conda import CondaCleanupMode
    ...
    group_internal.add_argument(
        ...
        help=help_internal("Set execution mode of SMK."),
    )
    group_internal.add_argument(
...
def generate_parser_metadata(parser, args):
    ...
    """
    command = "ismk %s" % " ".join(parser._source_to_settings["command_line"][""][1])
...
def parse_args(argv):
    ...
    if (args.profile or workflow_profile) and args.mode == ExecMode.DEFAULT:
        ...
        # But only do this if the user has invoked SMK (ExecMode.DEFAULT)
        profiles = []
        if args.profile:
...
def parse_wait_for_files(args):
    from ismk.io import wait_for_files
...
def args_to_api(args, parser):
    ...
    with SMKApi(output_settings) as ismk_api:
        ...
        try:
            ...
            if args.deploy_sources:
                ...
                workflow_api = ismk_api.workflow(

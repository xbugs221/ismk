...
from ismk import wrapper
from ismk.rules import Rule
from ismk.settings.types import DeploymentMethod

from ismk.template_rendering import check_template_output
from ismk.interfaces.common.utils import lazy_property
from ismk.interfaces.executor_plugins.jobs import (
...
from ismk.resources import GroupResources
from ismk.target_jobs import TargetSpec
from ismk.sourcecache import LocalSourceFile, SourceFile, infer_source_file
...
from ismk.common.tbdstring import TBDString
from ismk.interfaces.report_plugins.interfaces import JobReportInterface
    ...
    async def new(
        ...
        async def new():
            if update:
                ...
                def new_job():
                    return Job(rule, dag, wildcards_dict, format_wildcards, targetfile)
...
class Job(
    ...
    async def prepare(self):
        ...
        # Shallow simply symlink everything in the working directory.
            ...
            ismk_dir = os.path.join(cwd, ".ismk")
            for dirpath, dirnames, filenames in os.walk(cwd, followlinks=True):
                ...
                # Must exclude .ismk and its children to avoid infinite
                # loop of symlinks.
                if os.path.commonprefix([ismk_dir, dirpath]) == ismk_dir:
                    continue
            ...
            if (
                ...
                if kept_directory:
                    logger.error(
                        ...
                        + "Run ismk with --cleanup-shadow to clean up shadow directories."
                    )
...
class Reason:
    ...
    def clear(self) -> None:
        from ismk.persistence import NO_PARAMS_CHANGE
    ...
    def set_cleanup_metadata_instructions(self, job):
        self.cleanup_metadata_instructions = (
            "To ignore these changes, run ismk "

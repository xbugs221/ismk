...
from ismk.interfaces.common.exceptions import WorkflowError, ApiError
...
def cut_traceback(ex):
    lines = []
    ismk_path = os.path.dirname(__file__)
    not_seen_ismk = True
    for line in traceback.extract_tb(ex.__traceback__)[::-1]:
        ...
        def is_ismk_dir(path):
            return os.path.realpath(path).startswith(os.path.realpath(ismk_path))

        if is_ismk_dir(dir):
            not_seen_ismk = False
        if not os.path.isdir(dir) or not_seen_ismk:
            lines.append(line)
    return lines[::-1]
...
def format_exception_to_string(ex, linemaps=None):
    ...
    if origin is not None:
        ...
        return "Cancelling ismk on user request."
    else:
...
class IOException(RuleException):
    def __init__(self, prefix, job, files, include=None, lineno=None, snakefile=None):
        from ismk.logging import format_wildcards
    ...
    def __init__(self, job, files, include=None, lineno=None, snakefile=None):
        ...
        if any(map(lambda f: f.startswith("~"), files)):
            msg += (
                ...
                "specific syntax is not resolved by SMK. In general, "
...
class IncompleteFilesException(RuleException):
    def __init__(self, files):
        ...
        super().__init__(
            ...
            "    ismk --cleanup-metadata <filenames>\n\n"
...
class IncompleteCheckpointException(Exception):
    def __init__(self, rule, targetfile):
        super().__init__(
            ...
            "https://ismk.readthedocs.io/en/stable/"
            ...
            "by name, such that SMK can call it internally "
            "once the checkpoint is finished."
        )
        self.rule = rule
        from ismk.io import checkpoint_target
...
class LockException(WorkflowError):
    def __init__(self):
        super().__init__(
            ...
            "sure that no other SMK process is trying to create "
            "the same files in the following directory:\n{}\n"
            "If you are sure that no other "
            "instances of ismk are running on this directory, "

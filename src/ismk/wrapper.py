...
from ismk.sourcecache import GithubFile, SourceCache, infer_source_file
    ...
    if not is_url(path):
        ...
        if prefix is None:
            ...
            if ver_regex.match(ref):
                ...
                return GithubFile(repo="ismk/ismk-wrappers", tag=ref, path=path_tail)
            else:
                # Otherwise, use a plain url and store it in runtime cache only.
                path = "https://github.com/ismk/ismk-wrappers/raw/" + path
        else:
            path = prefix + path
...
def wrapper(
    ...
    """
    Load a wrapper from https://github.com/ismk/ismk-wrappers under

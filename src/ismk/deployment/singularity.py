...
SMK_MOUNTPOINT = "/mnt/ismk"


def get_ismk_searchpath_mountpoints():
    paths = get_ismk_searchpaths()
    base = Path("/mnt/ismk_searchpaths")
...
def shellcmd(
    ...
    if is_python_script:
        # mount host ismk module into container
        args += " ".join(
            f" --bind {str(searchpath)!r}:{str(mountpoint)!r}"
            for searchpath, mountpoint in zip(
                get_ismk_searchpaths(), get_ismk_searchpath_mountpoints()
            )
        )

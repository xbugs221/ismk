...
import ismk
from ismk import script, wrapper, notebook
from ismk.io.fmt import fmt_iofile
from ismk.jobs import Job
from ismk.report.common import data_uri_from_file, mime_from_file
...
from ismk.exceptions import InputFunctionException, WorkflowError
from ismk.script import SMK
from ismk.common import (
    get_input_function_aux_params,
)
from ismk import logging
...
from ismk.settings.types import GlobalReportSettings
...
from ismk.common import get_report_id
        ...
        if self.raw_caption is not None:
            ...
            try:
                ...
                caption = env.from_string(caption).render(
                    ismk=ismk, categories=categories, files=files
                )
        ...
        for f in job.output:
            ...
            if not meta:
                logger.warning(
                    ...
                    "version of SMK. This is a non critical "
                    "warning.".format(f)
                )
                continue
            ...
            try:
                workflow_description = publish_parts(
                    env.from_string(workflow_description).render(
                        ismk=SMK, categories=results, files=files
                    ),
                    writer_name="html",
                )[
                    "html_body"

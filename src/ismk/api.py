...
import tarfile
import uuid
from ismk.common import MIN_PY_VERSION, SNAKEFILE_CHOICES, async_run
...
from ismk.settings.enums import ChangeType

if sys.version_info < MIN_PY_VERSION:
    raise ValueError(
        f"SMK requires at least Python {'.'.join(map(str, MIN_PY_VERSION))}."
    )
...
from ismk.scheduling.milp import SchedulerSettings as IlpSchedulerSettings

from ismk.interfaces.executor_plugins.settings import ExecMode, ExecutorSettingsBase
from ismk.interfaces.executor_plugins.registry import ExecutorPluginRegistry
from ismk.interfaces.common.exceptions import ApiError
...
from ismk.workflow import Workflow
from ismk.exceptions import print_exception
from ismk.logging import logger, logger_manager
...
@dataclass
class SMKApi(ApiBase):
    """The SMK API.

    Arguments
    ---------

    output_settings: OutputSettings -- The output settings for the SMK API.
    """
    ...
    def print_exception(self, ex: Exception):
        ...
        traces that hide SMK internals for better readability).
    ...
    def _check_is_in_context(self):
        if not self._is_in_context:
            raise ApiError(
                "This method can only be called when SMKApi is used within a with "
                "statement."
            )
...
class WorkflowApi(ApiBase):
    ...
    def _get_workflow(self, **kwargs):
        ...
        return Workflow(
            ...
            output_settings=self.ismk_api.output_settings,
...
class DAGApi(ApiBase):
    ...
    def execute_workflow(
        ...
        updated_files: Optional[List[str]] -- An optional list where SMK will put all updated files.
        """
        ...
        if (
            ...
            raise ApiError(
                ...
                "See https://ismk.github.io/ismk-plugin-catalog for possible "
        ...
        if scheduler == "greedy" and scheduler_settings is None:
            # When directly using greedy scheduler, use greedy_scheduler_settings
            scheduler_settings = greedy_scheduler_settings

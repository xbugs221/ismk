...
import ismk.io
import ismk.utils
from ismk.exceptions import LookupError
...
class WildcardHandlerBase(ABC):
    ...
    def needs_wildcards(self, expression):
        return callable(expression) or any(
            ...
            for name in ismk.io.get_wildcard_names(expression)
        )
    ...
    def handle(self, expression):
        if self.needs_wildcards(expression) or any(
            ...
            def inner(wildcards):
                ...
                resolved_expression = ismk.utils.format(
            ...
            if self.namespace:
                resolved_expression = ismk.utils.format(expression, **self.namespace)
            else:
                resolved_expression = expression
...
class QueryWildcardHandler(WildcardHandlerBase):
    ...
    def apply_func(self, expression, namespace=None):
        ...
        if self.cols is not None and namespace is not None:
            if isinstance(self.cols, list):
                cols = [ismk.utils.format(col, **namespace) for col in self.cols]
            else:
                cols = ismk.utils.format(self.cols, **namespace)
...
def lookup(
    ...
    **namespace,
    ...
    In that case, this function returns a SMK input function which takes
    wildcards as its only argument and will be evaluated by SMK
